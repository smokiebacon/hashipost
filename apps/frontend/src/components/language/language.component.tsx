...
import { languages } from '@gitroom/frontend/app/i18n/settings'
import ReactCountyFlag from 'react-country-flag';
...
//   const { data, isLoading } = useSWR('notifications', loadNotifications);
    ...
    const changeLang = (langCode: string) => {
      // Convert string with array using pattern /
...
router
  return (
    <div id="notification-popup" className="opacity-0 animate-normalFadeDown mt-[10px] absolute w-[320px] min-h-[200px] top-[100%] right-0 bg-third text-textColor rounded-[16px] flex flex-col border border-tableBorder z-[20]">
      ...
      <div className="flex flex-col">
        {
          languages.map((lang, index) => (
            ...
            <div key={index} className="p-[16px] border-b border-tableBorder">
              <button onClick={() => {
                Cookies.set('lang', lang.langCode);
                // router.replace('/'+lang.langCode);
                changeLang(lang.langCode)
              }}>
              <ReactCountyFlag countryCode={lang.countryCode} svg />
              <span className="ml-[10px]">{lang.name}</span>
              </button>
            </div>
          ))
        }
...
const LanguageComponent = () => {
  ...
  console.log('query',router.lang, findLangauge)
      ...
      <div onClick={changeShow}>
        {
          findLangauge && (
            <div className="flex items-center">
              <ReactCountyFlag countryCode={findLangauge.countryCode} svg />
              <span className="ml-[10px]">{findLangauge.name}</span>
              </div>
          )
        }
        {/* <svg
